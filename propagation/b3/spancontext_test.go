...
func TestParseHeaderSuccess(t *testing.T) {
	testCases := []struct {
		...
		{
			"d",
			&model.SpanContext{Debug: true},
			nil,
		},
		{
			"1",
			&model.SpanContext{Sampled: pointerBool(true)},
			nil,
		},
		{
			"000000000000007b00000000000001c8-000000000000007b",
			&model.SpanContext{TraceID: model.TraceID{High: 123, Low: 456}, ID: model.ID(123)},
			nil,
		},
		{
			"000000000000007b00000000000001c8-000000000000007b-0",
			&model.SpanContext{TraceID: model.TraceID{High: 123, Low: 456}, ID: model.ID(123), Sampled: pointerBool(false)},
			nil,
		},
		{
	...
	for _, testCase := range testCases {
		...
		if testCase.expectedContext != nil {
			...
			if !(actualContext.TraceID == testCase.expectedContext.TraceID &&
				...
				t.Fatalf(
					"unexpected context for header %q, want: %v, have %v",
					testCase.header,
